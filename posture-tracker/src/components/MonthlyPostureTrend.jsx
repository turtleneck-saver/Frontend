...
function MonthlyPostureTrend() {
  ...
  const weekGroups = [
    postureRecords.slice(0, 2),
    postureRecords.slice(2, 4),
    postureRecords.slice(4, 5),
    postureRecords.slice(5, 7),
  ];

  const weekAverages = weekGroups.map(week =>
    Math.round(
      week.reduce((sum, d) => sum + d.goodPostureRatio, 0) / week.length
    )
  );
